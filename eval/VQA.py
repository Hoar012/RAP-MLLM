...
def main(args):
    ...
    for name, samples in tqdm(test_set.items()):
        for img_path, sample in samples.items():
            conv = conv_templates[args.conv_mode].copy()
            ...
            for option, content in sample["options"].items():
                q += f"{option}. {content}\n"
            
            q += 'Please give a single character from the options above. \n'
            
            if args.retrieval:
                extra_info, rag_images = retriever.retrieve(database, inp = q, queries = [], topK = args.topK)
            ...
            results.append({"img_path": img_path, "question": q, "option": sample["options"], "answer": outputs, "correct_answer": sample["correct_answer"]})
    ...
    with open(f"results/VQA-{save_time}.json", "w") as f:
